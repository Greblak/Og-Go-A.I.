...
const int GoBoard::CurrentPlayer() const
{
  return State.toPlay;
}
...
{
  if(p >= BOARD_TOP_LEFT || p < BOARD_BOTTOM_LEFT)
    return -1;
  return p+POS_NS;
}
...
{
  if(p <= BOARD_BOTTOM_RIGHT || p > BOARD_TOP_RIGHT)
    return -1;
  return p-POS_NS;
}
...
{
  if(p % Size() == 0 || p < BOARD_BOTTOM_LEFT)
    return -1;
  return p-POS_WE;
}
...
{
  if( p < BOARD_BOTTOM_LEFT || p > BOARD_TOP_RIGHT)
    return -1;
  if((p+1)%Size()==0 && p != 0)
    return -1;
  return p+POS_WE;
