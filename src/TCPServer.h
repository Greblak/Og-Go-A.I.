...
{
public: 
    inline TCPServer(boost::asio::io_service& io_service, int port, boost::function<void (boost::asio::ip::tcp::socket&)> acceptCallback): acceptor_(io_service, boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), port)), acceptCallback(acceptCallback)
    {
	prepareAccept();
    }

private:
    boost::function<void (boost::asio::ip::tcp::socket&)> acceptCallback;
